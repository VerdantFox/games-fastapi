...
def _register_blueprints_fixture() -> TestClient:
    """Register blueprints for the app."""
    register_blueprints(app)


@pytest.fixture(scope="session", autouse=True)
def _postgres_create_teardown_db_fixture() -> Generator[
    None, None, None
]:  # pragma: no cover
...
def _postgres_setup_teardown_db_fixture(
    engine: Engine,
) -> Generator[None, None, None]:  # pragma: no cover
    """Set up the test postgres database for each test and teardown afterwards."""
    if settings.in_mem_db:
        yield
        return
    yield
...
def client_fixture(session: Session) -> Generator[TestClient, None, None]:
    """Create a new test client for testing with a new session from the session fixture."""
